...
import serial
import time 
from trip_serial.trip_robot import TripRobot
...
class TripSerial(Node):
    ...
    def cmd_callback(self, msg):
        lin_vel_mps = msg.linear.x
        ang_vel_radps = msg.angular.z

        encoderd_cmd = self.Trip.encodeVehicleVel(lin_vel_mps, ang_vel_radps)
        self.ser.write(encoderd_cmd.encode('utf-8'))
    ...
    def __checkParams(self):
        ...
        if(max_motor_rpm == 0.0):
            raise InvalidParameterValueException('max_motor_rpm=',max_motor_rpm,". Motor max speed is 0.0! Change it to a positive value.")
        if(enc_period_s < 0.0):
    ...
    def shutdown(self):
        self.ser.close()
        self.get_logger().info(f'Closed communication on serial port.' , once=True)
